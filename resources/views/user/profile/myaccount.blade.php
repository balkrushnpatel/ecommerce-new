...
@push('stylesheets') 
<style type="text/css">
    .btn.btn-block{width: auto;}
    .page-link{display: inline;}
    :hover:not(.disabled) > .page-link, .active:not(.disabled) > .page-link,.pagination a{padding: 8px;border-radius: 50px}
    :hover:not(.disabled) > .page-link, .active:not(.disabled) > .page-link{background-color: #336699;color: #fff;}
</style>
@endpush
@section('content')
    <main class="main"> 
            ...
            <div class="container">
                <div class="tab tab-vertical row gutter-lg">
                    <ul class="nav nav-tabs mb-6" role="tablist">
                        <li class="nav-item">
                            <a href="#account-dashboard" class="nav-link active" id="dashboard">Dashboard</a>
                        </li>
                        <li class="nav-item">
                    ...
                    <div class="tab-content mb-6">
                                    ...
                                    @foreach($orders as $order)
                                        <tr>
                                            ...
                                            <td class="order-action">
                                                <a href="{{route('order.view',encrypt($order['id']))}}" class="btn btn-outline btn-default btn-block btn-sm btn-rounded" target="_blank">View</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="tab-pane" id="wishlist">
                            <div class="icon-box icon-box-side icon-box-light">
                                ...
                                <div class="icon-box-content">
                                    <h4 class="icon-box-title text-capitalize ls-normal mb-0">Wishlist</h4>
                                </div>
                            </div>                           
                            <table class="shop-table account-orders-table mb-6">
                                <thead>
                                    <tr>
                                        <th class="order-id">Indx</th>
                                ...
                                <tbody>
                                    @foreach(Auth::user()->userWishlist as $key=>$wishlist)
                                        <tr>
                                            <td class="order-id">{{ ($key + 1) }}</td>
                                            <td class="order-date">{{ $wishlist->product->categories->name}}</td>
                                            <td class="order-status">{{ $wishlist->product->name}} </td>
                                            <td class="order-total">
                                                {!! $wishlist->product->productPrice() !!}
                                            </td>
                                            <!-- <td class="order-action">
                                                <a href="{{ $wishlist->product->productSlug() }}" class="btn btn-outline btn-default btn-block btn-sm btn-rounded">View</a>
                                            </td> -->
                                            <td class="wishlist-action">
                                                <div class="d-lg-flex">
                                                    <a href="{{ $wishlist->product->productSlug() }}" class="btn btn-outline btn-default btn-block btn-sm btn-rounded mb-2 mb-lg-0">Quick View</a>
                                                    <a href="javascript:void(0);" class="btn btn-dark btn-rounded btn-sm ml-lg-2 btn-cart" onclick="to_cart({{ $wishlist->product_id}})">Add to cart</a>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
