...
@section('content')
@include('layouts.partials.sub-header',['pageHeader' => 'Sub Category'])
<!--begin::Entry-->
		...
		<div class="card card-custom">
			<div class="card-header flex-wrap py-3">
				...
				<div class="card-toolbar">  
					<!--begin::Button-->
					<a href="{{ route('subcategory.index') }}" class="btn btn-primary font-weight-bolder">
		  ...
		  <form id="subcategory-form"  class="form fv-plugins-bootstrap fv-plugins-framework" action="@yield('editId',route('subcategory.index'))" method="post" accept-charset="utf-8" role="form" enctype="multipart/form-data">
				{{csrf_field()}}
				@section('editMethod')
				@show  
				@csrf 
						  	 	...
						  	 	<div class="col-lg-12">
	  	 							<div class="radio-inline">
										...
										<label class="radio radio-success">
											<input type="radio" name="status" value="1" {{ (isset($subcategory) && ($subcategory->status == 1) ) ? 'checked': ''}} >
											<span></span>Active
										</label>
										<label class="radio radio-success">
										<input type="radio" name="status" value="0" {{ (isset($subcategory) && ($subcategory->status == 0) ) ? 'checked': ''}}>
				...
				<div class="card-footer">
					<div class="row">
						<div class="col-lg-12">
							<a href="{{ route('subcategory.index')}}" class="btn btn-light-primary font-weight-bold">Cancel</a>
							<button type="submit" class="btn btn-light-success font-weight-bold mr-2">{{ isset($subcategory) ? 'Update' : 'Save' }} </button>
						</div>
					</div>
				</div>
			</form>
...
@endsection
@push('scripts')  
<script src="{{ asset('wjs/subcategory.js')}}"></script>
@endpush